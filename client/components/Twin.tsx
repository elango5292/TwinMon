...
export default function Pipe({ data }) {
  ...
  const noz0 = useRef(null);
  const noz1 = useRef(null);
  const noz2 = useRef(null);
  const noz3 = useRef(null);
  const noz4 = useRef(null);

  const rod0 = useRef(null);
  const rod1 = useRef(null);
  const rod2 = useRef(null);
  const rod3 = useRef(null);
  const rod4 = useRef(null);
  const rod5 = useRef(null);

  const base0 = useRef(null);
  const base1 = useRef(null);
  const base2 = useRef(null);
  const base3 = useRef(null);
  const base4 = useRef(null);
  const base5 = useRef(null);
  ...
  useEffect(() => {
    ...
    if (noz0.current) {
      ...
      changeNozzlePositionAbsolute(positionAbsolute.y, positionAbsolute.z);
      changeRodPositionAbsolute(positionAbsolute.y);
      changeBasePositionAbsolute(positionAbsolute.x);
  ...
  function changeNozzlePositionAbsolute(y: number, z: number) {
    if (noz0.current && noz1.current && noz2.current && noz3.current && noz4.current) {
     
      noz0.current.position.y = y;
      noz0.current.position.z = z;


     
      noz1.current.position.y = y;
      noz1.current.position.z = z;


      
      noz2.current.position.y = y;
      noz2.current.position.z = z;


      
      noz3.current.position.y = y;
      noz3.current.position.z = z;


      
      noz4.current.position.y = y;
      noz4.current.position.z = z;
  ...
  function changeRodPositionAbsolute(y: number) {
    if (rod0.current && rod1.current && rod2.current && rod3.current && rod4.current && rod5.current) {
   
      rod0.current.position.y = y;
     


      rod1.current.position.y = y;
    


     
      rod2.current.position.y = y;
  


    
      rod3.current.position.y = y;
     

      rod4.current.position.y = y;
  

      rod5.current.position.y = y;

      console.log(rod0.current.position, rod1.current.position, rod2.current.position, rod3.current.position, rod4.current.position,rod5.current.position)
    }
  }

  function changeBasePositionAbsolute(x: number) {
    if (base0.current && base1.current && base2.current && base3.current && base4.current && base5.current) {
      base0.current.position.x = x;
     


      base1.current.position.x = x;
     


      base2.current.position.x = x;
      


      base3.current.position.x = x;
      


      base4.current.position.x = x;
      
      base5.current.position.x = x;
  ...
  function transformCoordinates(x: number, y: number, z: number) {
    ...
    const minX1 = 0, maxX1 = 100;
    const minY1 = 0, maxY1 = 100;
    const minZ1 = 0, maxZ1 = 100;
